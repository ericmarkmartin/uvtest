...
for i in range(N):
    Uxs.append(nkp.operator.Rx(hi, i, 2 * dt * h))
    Uxs_dagger.append(nkp.operator.Rx(hi, i, 2 * dt * h))
...
S2op = nkp.Renyi2EntanglementEntropy(hi, subsys)
...
def dynamics_with_measurements(
    ...
    for t in ts:
        ...
        for i in range(len(Uxs)):
            te = nkp.driver.InfidelityOptimizer(
                phi,
                optimizer,
                U=Uxs[i],
